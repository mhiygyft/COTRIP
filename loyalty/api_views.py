...
class LoyaltyMembershipViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['status', 'tier__name']
    search_fields = ['member_id', 'user__email', 'user__first_name', 'user__last_name']
    ordering_fields = ['points_balance', 'lifetime_points', 'joined_at']
    ...
    def stats(self, request, pk=None):
        ...
        if next_tier:
            points_to_next_tier = max(0, next_tier.min_points - membership.tier_qualifying_points)
        ...
        stats_data = {
            ...
            'tier_qualifying_points': membership.tier_qualifying_points,
            'current_tier': membership.tier,
            'next_tier': next_tier,
            'points_to_next_tier': points_to_next_tier,
            'tier_expires_at': membership.tier_expires_at,
    ...
    def tier_progress(self, request, pk=None):
        ...
        if next_tier:
            points_needed = max(0, next_tier.min_points - membership.tier_qualifying_points)
            progress_percentage = min(100, 
                (membership.tier_qualifying_points - current_tier.min_points) / 
...
class PointsTransactionViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['description', 'booking_reference']
...
class RewardViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['category', 'reward_type', 'is_active', 'min_tier_required']
    search_fields = ['name', 'description']
    ...
    def public(self, request):
        ...
        rewards = self.queryset.filter(is_active=True)
    ...
    def available(self, request):
        ...
        try:
            ...
            for reward in self.queryset.filter(is_active=True):
                can_redeem = True
                reason = None
                points_needed = None
                tier_required = None
                ...
                if reward.min_tier_required and reward.min_tier_required.min_points > membership.tier.min_points:
                    can_redeem = False
                    reason = "Tier requirement not met"
                    tier_required = reward.min_tier_required
...
class RewardRedemptionViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['redemption_code', 'reward__name']
    ordering_fields = ['redeemed_at', 'points_used']
...
class PromotionUsageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = ['used_at', 'points_earned', 'bonus_points']
    ordering = ['-used_at']
